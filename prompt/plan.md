...
### PR #11: 日本語ファイル名対応 (UTF-8)

**目的:** リポジトリの設定に関わらず、日本語UTF-8ファイル名を `pickit` が正しく扱えるようにする。これにより、日本語ファイル名を含むプロジェクトでの利用が可能になり、ユーザビリティを向上させる。
...
4.  **テストケースの追加:**
    *   日本語ファイル名とディレクトリ名を含む新しいテスト用Gitリポジトリを自動生成するテストスイートを追加する。
