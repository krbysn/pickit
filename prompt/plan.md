...
`pickit` は Git の疎結合チェックアウトを管理するツールであるため、パス処理においては Git のセマンティクスを最優先する。OS ネイティブのパス表現 `PathBuf` は、アプリケーション起動時のリポジトリルート指定および `std::env::set_current_dir` の呼び出しに限定し、それ以外のリポジリ内部パスの表現には Git の出力形式に合わせた `String` を用いる。これにより `std::os::unix::ffi::OsStringExt` の利用を排除し、Windows を含むクロスプラットフォームでの堅牢な動作を目指す。

## 詳細

1.  **リポジトリ内部パスの内部表現:**
    *   `TreeItem.path`, `App` 構造体内の `sparse_checkout_dirs`, `uncommitted_paths` など、リポジリ内のファイルやディレクトリを示すパスは、**クオートされた Git の文字列表現**を `String` 型で保持する。
...
2.  **Git コマンドの実行とパスの取得:**
    ...
    *   現在の `git ls-tree -z`, `git diff -z`, `git ls-files -z` の呼び出しを、対応する**非 `--zero` オプションのコマンド**に変更する。これらはパスを改行区切りで出力し、必要に応じてクオートされる。
        *   例: `git ls-tree -r --name-only -d HEAD` → `git ls-tree -r --name-only -d HEAD` (ただし、これだけでは `core.quotepath` が適用されない可能性や、ディレクトリとファイルが混じる問題があるため、適切なコマンド選定が必要)
...
Git の `core.quotepath=true` によるクオート形式を内部表現に用いることで、クロスプラットフォームかつ安全なパス処理を目指したが、以下の問題により機能不全に陥った。

*   **Git コマンドの入力と出力の不整合:** `git` コマンド間でのパスのクオート/アンクオートのセマンティクスが複雑かつ一貫せず、特に `ls-tree HEAD:<path>` や `sparse-checkout set` の引数処理で問題が発生した。
*   **非 ASCII パス（特に日本語）の取り扱い:** Git の `core.quotepath` が出力するクオート形式が、Rust の `std::process::Command` や TUI ライブラリとの間で正しくやり取りされない、または正しくデコード/エンコードされない問題が解決できなかった。

これらの問題により、アプリケーションの主要機能（フォルダの展開、チェックアウト状態の表示）が継続的に機能しないため、方針を根本的に転換する。

## 新しい方針 (2026年2月11日)

内部データは、**エスケープを外した（デコードされた）純粋な UTF-8 文字列**として管理する。Git コマンドとのやり取りにおいては、Git が期待するパス形式を優先し、必要に応じて明示的なエンコード/デコードを行う。`std::os::unix::ffi::OsStringExt` の利用を完全に排除し、クロスプラットフォームでの動作を目指す。

## 詳細

1.  **リポジトリ内部パスの内部表現:**
    *   `TreeItem.path`, `App` 構造体内の `sparse_checkout_dirs`, `uncommitted_paths` など、リポジトリ内のファイルやディレクトリを示すパスは、**エスケープを外した純粋な UTF-8 `String`** 型で保持する。
    *   パス区切り文字は常に `/` を使用する。

2.  **Git コマンドの実行とパスの取得:**
    *   `src/git.rs` 内で `git` コマンドを実行する際、`std::process::Command` に `"-c", "core.quotepath=false"` を明示的に設定する。これにより Git からの出力がクオートされないようにする。
    *   現在の `git ls-tree`, `git diff`, `git ls-files` の呼び出しを、**非 `--zero` オプションのコマンド**に変更する。これらはパスを改行区切りで出力する。
    *   `git` コマンドの標準出力 (`stdout`) は `Vec<u8>` として受け取る。これを `String::from_utf8_lossy()` で `String` に変換する。この `String` は、改行区切りで**エスケープされていない純粋な UTF-8 パス**を含む。
    *   この `String` を改行で分割し、各行を内部のパス表現 (`String`) として格納する。

3.  **リポジトリ内部パスの操作（join, parent, file_name, starts_with など）:**
    *   これらの操作は、内部の**エスケープされていない UTF-8 `String`** に対して直接行う。
    *   `String::rfind('/')` で最後の区切り文字を見つけ、`String::slice` で親パスやファイル名を抽出するなどの標準的な `String` 操作を用いる。
    *   これらのロジックは `pickit` 内にカスタムで実装する、または Rust の `PathBuf` メソッドがより適切に利用できるか検討する (ただし `PathBuf` への変換・からの変換コストは考慮する)。

4.  **表示（TUI）:**
    *   内部に保持するエスケープされていない UTF-8 `String` パスを**そのまま** TUI に表示する。`unescape_git_path_string` 関数は不要になる。

5.  **Git コマンドへの入力（引数として）:**
    *   `git sparse-checkout set` など、内部のパスを `git` コマンドの引数として渡す必要がある場合、内部の**エスケープされていない UTF-8 `String` パスをそのまま** `std::process::Command::arg()` メソッドに渡す。
    *   特にスペースや特殊文字を含むパスについては、`Command::arg()` が自動的にシェルセーフなクォートを適用することを期待する。

6.  **`PathBuf` の利用:**
    *   `current_repo_root: PathBuf` は `App` 構造体で引き続き `PathBuf` 型として保持する。これは `std::env::set_current_dir` や `std::process::Command::current_dir` に必要であるため。
    *   `git::get_dirs_at_path` で `current_dir` として渡すパスは `Path` または `PathBuf` である必要があるため、ここでの `PathBuf` の利用は維持する。

## PR への影響 (再改訂)

この新しいパス処理戦略は、既存のPR計画に以下の変更をもたらす。

*   **PR #1:** `git` モジュール内の Git コマンド呼び出しとその出力処理が大幅に変更される。特に `core.quotepath=false` の設定と、Git 出力パスの**直接的な UTF-8 デコード**が含まれる。`unescape_git_path_string` 関数は不要になるか、大幅に簡素化される。
*   **PR #2, #3, #4, #5, #6:** `TreeItem.path` などのパス関連フィールドの型は `String` のままだが、その内容が**エスケープされていない純粋な UTF-8 パス**になる。パス操作ロジックは標準的な `String` 操作に依存し、`unescape_git_path_string` への呼び出しは全て削除される。`App` 内でのパス比較はエスケープされていない `String` 同士で行われる。
*   **PR #10 (日本語ファイル名対応):** このPRで提案されていたロケール設定の強化や UTF-8 デコードの確実化は、新しい戦略（`core.quotepath=false` 設定と `String::from_utf8_lossy` での直接的な UTF-8 変換）によって達成される。
