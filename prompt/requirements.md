...
- **白 (White):**
    - 上位のディレクトリがチェックアウトされているため、間接的にチェックアウトされている状態。
    - または、チェックアウトされていないが (`☐`)、下位にチェックアウト済みのディレクトリを含む (`·`)。
- **灰色 (DarkGray):** チェックアウトされておらず (`☐`)、下位にもチェックアウト済みのディレクトリを含まない。
...
- `Up` / `Down` キー: ツリービューでカーソルを上下に移動します。
- `Right` キー:
    *   現在のノードが閉じている場合、そのノードを展開します。
    *   現在のノードが開いている場合は、何もしません（展開専用）。
- `Left` キー:
    *   現在のノードが開いている場合、そのノードを収縮します。
    *   現在のノードが閉じている場合、選択を親ノードに移動します（親ノードに移動後、親ノードが閉じている場合はその親ノードも収縮させます）。
- `PgUp` / `PgDown` キー:
    *   `PgUp` キーで、ビューポートを1ページ分上にスクロールし、選択を新しいビューポートの最初の表示可能ノードに移動します。
    *   `PgDown` キーで、ビューポートを1ページ分下にスクロールし、選択を新しいビューポートの最後の表示可能ノードに移動します。
- `Home` / `End` キー:
    *   `Home` キーで、ツリーの最初のノードに移動します。
    *   `End` キーで、ツリーの最後のノードに移動します。
- `Ctrl + Up` / `Ctrl + Down` キー: ツリーのルートノードにジャンプするか、表示されているツリーの端にジャンプします (提案機能)。
- `/` キー: 現在のツリー内でノードをインクリメンタル検索し、一致するノードにジャンプします (提案機能)。
- `Tab`: ツリービューとグリッドビューのフォーカスを切り替えます。
