...
-   **説明:** 大型ディレクトリにおいて、上下キー操作時の応答が著しく遅延し、アプリケーションの利用ストレスが大きい。特にディレクトリ階層が深く、ファイル数が多い場合に顕著である。**以前の変更でパフォーマンスの改善は見られたものの、未だ不十分であり、ファイル監視機能がその原因であると特定された。**
-   **影響範囲:** TUIのファイルツリー表示、キー操作によるナビゲーション、ユーザーエクスペリエンス全般。
-   **優先度:** 高
-   **提案される修正アプローチ:**
    -   **自動ファイル監視機能の完全に削除する。** これにより、不要なリソース消費とパフォーマンスオーバーヘッドを排除する。
    -   代わりに、**`r` キーによる手動リフレッシュ機能を追加する。** ユーザーは必要に応じて `r` キーを押すことで、表示されているツリーの状態を最新の情報に更新できる。
    -   手動リフレッシュ機能の実装に伴い、`App` 層に、アプリケーションの状態を再読み込みし、TUIを再描画するための `refresh` メソッドを実装する。
    -   パフォーマンス改善が意図通りに行われたことを確認するための簡単なプロファイリングまたはベンチマークを行う。
    -   関連するコード（`src/app.rs`, `src/git.rs` など）のレビューと最適化。
-   **チェックリスト:**
    -   [ ] 自動ファイル監視機能のコードを完全に削除
    -   [ ] `r` キーによる手動リフレッシュ機能のTUIハンドリングを実装
    -   [ ] `App` 層に `refresh` メソッドを実装
    -   [ ] パフォーマンス改善の確認（プロファイリング/ベンチマーク）
