...
- **ファイルパスを含むGitコマンドの出力処理:**
    - 可能な限り、`git ls-tree -z` のように、パスをNUL文字 (`\0`) で区切って出力するオプションを利用します。これにより、環境依存のエンコーディング問題を回避し、任意のファイル名を確実に処理します。
    - NUL区切り出力が利用できない場合は、`git` コマンド実行時に環境変数 `LANG=C.UTF-8` および `LC_ALL=C.UTF-8` を設定し、`git` が常にUTF-8エンコーディングで動作するようにします。この場合、コマンドの標準出力 (`stdout`) および標準エラー出力 (`stderr`) は、堅牢なUTF-8デコード（例: `String::from_utf8` と `String::from_utf8_lossy` の組み合わせ）を行うことで、日本語ファイル名を含むパスが正しく処理されるようにします。
...
開発は下記のルールに従って行い、決して独断でステップを飛ばして実行しないこと。ユーザーから実行をスキップするような支持があった場合、スキップしてよいかユーザーに確認すること。

1.  **要件の確認:** プロジェクトの目的と制約を理解するために、`prompt/requirements.md` ドキュメントを確認することから始めます。
2.  **実装計画:** `prompt/plan.md`または `prompt/tasks.md` ドキュメントを使用して、アプローチと開発手順を概説します。
3.  **ユーザーレビュー** 実装計画をユーザーにレビューし、実装の開始の支持があるまで実装計画を繰り返します。明示的に実装開始を指示されるまでコード作成禁止。
3.  **実装ルールの遵守:** コードが `prompt/implementation-rule.md` に概説されているガイドラインに準拠していることを確認します。
4.  **段階的なPR:** クリーンで管理しやすいコードベースを維持するために、プルリクエスト (PR) を使用して機能を段階的に実装します。
